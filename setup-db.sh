...
# Запустите этот скрипт на сервере: bash setup-db.sh

echo "Начинаем настройку PostgreSQL..."

# Подключаемся к PostgreSQL и выполняем команды
psql -U postgres <<EOF

-- Создание пользователя adminmfo
DO
\$do\$
BEGIN
   IF NOT EXISTS (
...
TRUNCATE TABLE mfo_companies RESTART IDENTITY;
